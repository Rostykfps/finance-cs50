...
def buy():
    ...
    if request.method == "GET":
        ...
        if stock == None:
            return apology("Symbol dosn't exist")
...
def register():
    ...
    if request.method == "GET":
        ...
        if not username:
            return apology("Please, give username")
        if not password:
            return apology("Please, give password")
        if not confirmation:
            return apology("Please, confirm your password")
        if password != confirmation:
            return apology("Password doesn't confirm")

        hash = generate_password_hash(password)
        try:
            new_user = db.execute("INSERT INTO users (username, hash) VALUES (?, ?)", username, hash)
        except:
            return apology("Username alreasdy exists")
...
def sell():
    ...
    if request.method == "GET":
        ...
        if shares > user_shares_real:
            return apology("You don't have enough of shares")
        ...
        flash("You sold action!")
        return redirect("/")

@app.route("/refill_balance", methods=["GET", "POST"])
@login_required
def refill_balance():
    """User can refill balance"""
    if request.method == "GET":
        return render_template("refill.html")
    else:
        add_cash = int(request.form.get("add_cash"))
        if not add_cash:
            return apology("You must refill balance")
        ...
        update_cash = round(user_cash + add_cash, 2)
        db.execute("UPDATE users SET cash = ? WHERE id = ?", update_cash, user_id)
